...
class FriendListViewAll(TemplateView):
    """
    displays existing social friends of the current user
    """
    template_name = "social_friends_finder/friend_list_all.html"
    ...
    def get(self, request, provider=None):
        ...
        if self.social_auths.count() == 0:
            if REDIRECT_IF_NO_ACCOUNT:
                return HttpResponseRedirect(REDIRECT_URL)
            return super(FriendListViewAll, self).get(request)
    ...
    def get_context_data(self, **kwargs):
        ...
        if all fail, redirects to the next page
        """
        context = super(FriendListViewAll, self).get_context_data(**kwargs)
        friends = []
        for friend_list in self.social_friend_lists:
            friends_info = friend_list.fetch_social_friends_info()
            friends.append(friends_info)

        # Add friend_ids to context
        context['friends'] = friends
        connected_providers = []
        for sa in self.social_auths:
            connected_providers.append(sa.provider)
